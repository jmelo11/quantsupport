...
//             .iter()
//             .filter(|quote| {
//                 identifiers.is_empty() || identifiers.contains(&quote.details().identifier())
//             })
//             .cloned()
//             .collect();
...
//         let rate_definition = index_details
//             .rate_definition()
...
//         let quotes =
//             match self.collect_quotes(ctx.evaluation_date(), &quotes, self.preset.instruments()) {
//                 Ok(values) => values,
//                 Err(_) => Vec::new(),
//             };
