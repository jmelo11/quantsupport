...
pub trait Pricer: Send + Sync {
    ...
    ///
    /// Evaluates the instrument over a [`Request`] given a [`crate::core::contextmanager::ContextManager`].
    ///
    /// ## Arguments
    ...
    /// Attaches a [`crate::core::collateral::DiscountPolicy`] that overrides default discounting and
    /// can define both discount-curve and pricing-currency resolution.
    fn set_discount_policy(&mut self, _policy: Box<Self::Policy>);

    /// Returns the currently active [`crate::core::collateral::DiscountPolicy`], if any.
